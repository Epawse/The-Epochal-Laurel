...
NEXT_STEP_REMINDER_POLICY = """<trellis-next-step-reminder>
At important workflow boundaries, include one concise visible Chinese line that starts with `下一步：` and tells the user the next Trellis workflow action.
Use the current `<task-status>` plus later per-turn `<workflow-state>` as the source of truth.
Trigger this reminder when planning is ready for `task.py start`, before/after edits, after checks pass or fail, before spec update/commit, after a clean commit, and when choosing between `/trellis:ship` and `/trellis:finish-work`.
Keep the reminder short and practical; do not turn it into a long checklist, and skip it only for tiny answers where no workflow state changed.
</trellis-next-step-reminder>"""
...
def main() -> None:
    ...
    raw_cwd = hook_input.get("cwd") or "."
    if not isinstance(raw_cwd, str):
        raw_cwd = "."
    cwd = Path(_normalize_windows_shell_path(raw_cwd)).resolve()
    project_dir = find_trellis_root(cwd)
...
""")
    ...
    output.write("\n\n")
    output.write(NEXT_STEP_REMINDER_POLICY)
    output.write("\n\n")

    output.write("<current-state>\n")
    output.write(_build_compact_current_state(trellis_dir, hook_input, spec_index_paths))
    try:
        sys.path.insert(0, str(trellis_dir / "scripts"))
        from common.backlog_context import get_backlog_armed_line  # type: ignore[import-not-found]

        armed = get_backlog_armed_line(project_dir)
        if armed:
            output.write(f"\n{armed}")
    except Exception:
        pass
    ...
    from common.setup_hints import build_local_setup_hints

    local_setup_hints = build_local_setup_hints(project_dir)
    if local_setup_hints:
        output.write(local_setup_hints)
        output.write("\n\n")

    from common.routing_hints import build_routing_hints

    output.write(build_routing_hints())

    from common.memory_context import build_memory_block

    memory_block = build_memory_block(project_dir)
    if memory_block:
        output.write("\n\n")
        output.write(memory_block)
    output.write("\n\n")
