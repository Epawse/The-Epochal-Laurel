...
`get_context.py` shows the active task's `status` field. Route by `status` + artifact presence + planning gate result. This command replaces the user needing to remember the Trellis flow; it does not itself approve implementation.

When `status=planning`, run the non-mutating planning gate before recommending `task.py start`:

```bash
python3 ./.trellis/scripts/task.py validate <task-dir>
```

If the output includes `Planning gate: 未通过`, route back to the missing planning step instead of asking for start.
...
- `status=planning` + P0/P1 platform workflow task missing `research/*.md` → **1.2** (write research artifact)
- `status=planning` + complex artifacts complete + sub-agent jsonl not curated (only the seed `_example` row) and task artifacts do not explicitly say no extra injected context is needed → **1.3**
- `status=planning` + required artifacts complete + required jsonl curated, explicit no-extra-context exception recorded, or inline mode → **1.4** (ask for start review; only run `task.py start` after user confirms)
